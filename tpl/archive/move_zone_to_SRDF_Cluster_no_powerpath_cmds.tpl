...
# if powermt is not installed, then get the disk ID's from the disk group

symdg show $zone_name | awk '/\(STD\)/,/RDF Info/'|awk '/DEV/ {print $3}'| sed 's/^0//' | tee ${tmp_folder}/device_ids.txt
SymmetrixID=`symdg list| awk '/'$zone_name'/ {print $4}'` && echo $SymmetrixID
DEVLIST=`symdg show $zone_name| awk '/\(STD\)/,/RDF Info/'|awk '/DEV/ {print $3}'| sed 's/^0//'` && echo $DEVLIST
for DEV in $DEVLIST; do WWN=`symdev show -sid $SymmetrixID $DEV | grep 'Device WWN' | awk '{print $4}'| sort -u` && echo "$WWN;$DEV";done

# if powermt is not installed, then no device id present in the storage_info   
# Manual entry for the storage array

cat ${tmp_folder}/storage_info_`uname -n`.txt| awk '{print substr($18,0,4)}'| sed '1d'| sort -u | tee storage_array.txt
...
zlogin ${zone_name}
for application in `ls /applications | grep -v wood | sed "s/\///g"`; do /applications/${application}/users/system/init.d/${application} status 2>/dev/null; done
...
# If powermt is not installed
{
for id in `cat wwns.txt | awk '{print substr($1,25)}'`
do     
     grep $id ${tmp_folder}/storage_info_`uname -n`.txt | awk '{print $3}' | while read disk     
     do       
...
# also echo | format 
tail -1000 /var/adm/messages| grep -i  'changed to OK'
...
zonecfg -z ${zone_name} -f ${tmp_folder}/${zone_name}.cfg && zoneadm list -civ | grep $zone_name
...
{
zoneadm list -civ | grep $zone_name
